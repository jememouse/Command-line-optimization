...
# 禁用 Oh My Zsh 的默认别名
export ZSH_DISABLE_ALIASES=true
...
list_files_long() {
    if command -v eza &> /dev/null; then
        ...
        exa -l --icons --group-directories-first --time-style=long-iso "$@"
    else
        ls -l "$@"
    fi
}
...
list_files_all() {
    if command -v eza &> /dev/null; then
        eza -la --icons --group-directories-first --time-style=long-iso "$@"
    elif command -v exa &> /dev/null; then
        exa -la --icons --group-directories-first --time-style=long-iso "$@"
    else
        ls -la "$@"
    fi
}
...
if command -v fd &> /dev/null; then
    alias file_finder='fd'
fi

if command -v rg &> /dev/null; then
    alias text_searcher='rg'
fi
...
go_to_grandparent_directory() {
    cd ../..
}

go_to_great_grandparent_directory() {
    cd ../../..
}
...
if [[ $- == *i* ]]; then
    # 检查是否是新的终端会话，且不在 powerlevel10k instant prompt 模式下
    if [[ -z "$TMUX" ]] && [[ "$TERM_PROGRAM" != "vscode" ]] && [[ -z "${POWERLEVEL9K_INSTANT_PROMPT+x}" ]]; then
        # 不显示任何欢迎信息，保持终端清洁
        echo "  • fuzzy_history_search     - 历史命令搜索"
        echo "  • find_file_by_name        - 按名称查找文件"
        echo "  • search_content           - 搜索文件内容"
        echo "  • fuzzy_process_kill       - 进程管理"
        echo "        # 不显示任何功能提示，保持终端清洁
    fi
fi
